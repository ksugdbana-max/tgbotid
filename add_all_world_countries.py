"""
Add ALL world countries to database - 195 countries
Run this to populate the Countries table with all world countries
"""
import asyncio
...
async def add_all_countries():
    print("\n" + "="*70)
    print("ADDING ALL WORLD COUNTRIES TO DATABASE")
    print(f"Total countries to add: {len(ALL_COUNTRIES)}")
    print("="*70)
    ...
    async with async_session() as session:
        ...
        for country_data in ALL_COUNTRIES:
            ...
            if existing:
                # Update if needed
                existing.flag = country_data["flag"]
                existing.code = country_data["code"]
                updated += 1
                print(f"✓ Updated: {country_data['flag']} {country_data['name']}")
            else:
                # Add new country
                new_country = Country(
                ...
                added += 1
                print(f"+ Added:   {country_data['flag']} {country_data['name']}")
    ...
    print(f"✅ Total: {len(ALL_COUNTRIES)} countries in database")
    print("="*70)
    print("\nAll countries will now show in admin panel!")

if __name__ == "__main__":
    asyncio.run(add_all_countries())
